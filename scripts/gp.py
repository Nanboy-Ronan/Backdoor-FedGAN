...
parser.add_argument('--grad_penalty', type=float, default=10, help ='gradient penalty coefficient')
...
print("This is the Defence Federated Learning")
if args.avg_disc:
...
def train(netD, netG, optimizerD, optimizerG, criterion, dataloader, device, args, attack=False):
    ...
    for data in dataloader:
        ...
        discriminator_output_real = netD(real_images)
        errD_real = torch.mean(discriminator_output_real)
        ...
        fake = netG(z, c)        
        # Calculate W-dic gradient penalty
        gradient_penalty = calculate_gradient_penalty(netD,
                                                    real_images.data, fake.data,
                                                    device)
        # Classify all fake batch with D
        discriminator_output_fake = netD(fake.detach())
        errD_fake = torch.mean(discriminator_output_fake)
        D_G_z1 = discriminator_output_fake.mean().item()
        errD = -errD_real + errD_fake + gradient_penalty*args.grad_penalty
        ...
        for j in range(args.gen_iters):
            ...
            discriminator_output_fake = discriminator_output_fake
            errG = -torch.mean(discriminator_output_fake)
...
def communication(server_disc, server_gen, disc_models, gen_models, client_weights, args):
    with torch.no_grad():
        if args.avg_disc:
...
for model_indx in range(n_disc):
    optimizer_d.append(optim.RMSprop(discriminator_model[model_indx].parameters(), lr=lr))

for model_indx in range(n_disc):
    optimizer_g.append(optim.RMSprop(generator_model[model_indx].parameters(), lr=lr))
...
for epoch in range(n_epochs):
    ...
    server_disc, server_generator, discriminator_model, generator_model = communication(server_disc, server_generator, discriminator_model, generator_model, client_weights, args)
